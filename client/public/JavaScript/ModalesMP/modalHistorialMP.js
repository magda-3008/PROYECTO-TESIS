async function abrirHistorialMP(ingrediente) {
    document.getElementById("nombreMateriaPrimaHistorial").textContent =
        ingrediente.nombre;
    ...
    try {
        const respuesta = await fetch(
            `/api/historial/${ingrediente.id_ma}?anio=${anio}&mes=${mes}`,
        );
        if (!respuesta.ok) {
        ...
        if (!movimientos || movimientos.length === 0) {
            ...
            movimientos.forEach((mov) => {
                ...
                switch (mov.tipo_movimiento) {
                    ...
                    case "CONSUMO":
                        tipoBadge =
                            '<span class="badge bg-warning text-dark">Consumo</span>';
                        montoTexto = formatoMoneda(mov.costo_total);
                        break;
                    case "PERDIDA":
                ...
                if (mov.tipo_movimiento === "PERDIDA" || mov.tipo_movimiento === "CONSUMO") {
                    cantidadMostrar = `<span class="text-danger">-${cantidad}</span>`;
                } else {
                    cantidadMostrar = `<span class="text-success">+${cantidad}</span>`;
                }
