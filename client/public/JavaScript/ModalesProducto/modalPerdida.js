...
async function registrarPerdida() {
	...
	if (!productoSeleccionado) {
		document.getElementById('errorGeneralPerdida').textContent = 'Debe seleccionar un producto.';
		return;
	}
	...
	if (!selectPeriodo || !selectPeriodo.value) {
		document.getElementById('errorGeneralPerdida').textContent = 'Debe seleccionar un período válido.';
		return;
	}
	...
	if (isNaN(cantidad) || cantidad <= 0) {
		mostrarErrorCampoPerdida('cantidadPerdida', 'errorCantidadPerdida', 'Ingrese una cantidad mayor a 0.');
		esValido = false;
	} else if (cantidad > productoSeleccionado.stock_actual) {
		mostrarErrorCampoPerdida('cantidadPerdida', 'errorCantidadPerdida', 'La cantidad no puede superar el stock actual.');
		esValido = false;
	}
	// 3. Validar Motivo
	...
	if (!motivo) {
		mostrarErrorCampoPerdida('motivoPerdida', 'errorMotivoPerdida', 'Describe el motivo de la pérdida.');
		esValido = false;
	}
	if (!esValido) return;
	const datosPerdida = {
	...
	try {
		...
		if (!response.ok) throw new Error(data.error || 'Error al registrar la pérdida.');
		// Cerrar Modal
...
if (modalPerdidaEl) {
	modalPerdidaEl.addEventListener('hidden.bs.modal', limpiarErroresModalPerdida);
}
document.getElementById('guardarPerdida').addEventListener('click', registrarPerdida);
